...
class CodeWriter:
    ...
    def emit(self, code_block, ensure_trailing_new_line):
        ...
        for part in code_block.format_parts:
            if part == "$L":
                self.emit_literal(code_block.args[a + 1])
            elif part == "$N":
                self.emit_and_indent(code_block.args[a + 1])
            elif part == "$S":
                string = str(code_block.args[a + 1])
                # Emit null as a literal null: no quotes.
                if string:
                    self.emit_and_indent(
                        Util.string_literal_with_double_quotes(string, self.indent))  # Util not finish yet
                else:
                    self.emit_and_indent("null")
            elif part == "$T":
